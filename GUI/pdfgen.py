...
class pdfgen:
    def __init__(self):
        ...
        # List to store selected input files
        self.input_files = []
        self.output_filename = ""
        self.elements = []

    def wrap_text(self, content, font_size=13):
        """Converts text with newlines to a Paragraph for wrapping."""
        # Create a custom paragraph style for consistent font size
        paragraph_style = self.styles["BodyText"].clone('custom')
    ...
    def read_questions(self):
        """Reads questions from input text files and categorizes them."""
    ...
    def create_page(self, questions):
        """Generates a multi-page PDF containing the model tests."""
        line1_style = ParagraphStyle('line1', parent=self.styles['Normal'], fontName='Times-Bold', fontSize=16, alignment=1, leading=25)
        line2_style = ParagraphStyle('line2', parent=self.styles['Normal'], fontName='Times-Roman', fontSize=14, alignment=1, leading=20)
        ...
        self.elements.append(Spacer(1, 20))  # Add space after header

        # Prepare the table data dynamically
        data = [["Type", "Questions"]]  # Header row

        # Updated rows with font size 14
        knowledge_rows = [["Knowledge-based", self.wrap_text(f'{i+1}. ' + q["question"], font_size=14)] for i, q in enumerate(questions["Knowledge"])]
        analytical_rows = [["Analytical", self.wrap_text(f'{i+1}. ' + q["question"], font_size=14)] for i, q in enumerate(questions["Analytical"])]
        problem_rows = [["Problem-based", self.wrap_text(f'{i+1}. ' + q["question"], font_size=14)] for i, q in enumerate(questions["Problem"])]
        ...
        self.elements.append(questions_table)
        self.elements.append(PageBreak())

        # Step 2: Create model tests

    def create_pdf(self, cards, questions):
        self.doc = SimpleDocTemplate(self.output_filename, pagesize=A4)
        self.elements = []
        for i in range(cards):
            ...
            self.create_page(selected_questions)
            yield (i*100)/cards
